  ...
  const saveLiveClassMutation = useMutation({
    ...
    onMutate: async () => {
      ...
      const newLiveClassOpt: LiveClass = {
        ...
        title: liveTitle,
        course: Number(liveCourse),
        course_title: courses.find(c => c.id === Number(liveCourse))?.title || '',
        scheduled_time: liveTime,
        meeting_url: liveMeetingUrl,
        status: editingLiveClass ? liveStatus : 'UPCOMING'
      };
            ...
            <div key={lc.id} className="rounded-2xl border border-border bg-card p-5 shadow-sm hover:shadow-md transition-all flex flex-col justify-between">
              <div>
                ...
                <span className="text-[10px] text-muted-foreground font-semibold mt-1 block">
                  {lc.course_title || `Course #${lc.course}`}
                </span>
                {(() => {
                  const courseObj = courses.find(c => c.id === lc.course);
                  return courseObj?.mentor_name ? (
                    <span className="text-[10px] text-primary/80 font-semibold mt-0.5 block">Mentor: {courseObj.mentor_name}</span>
                  ) : null;
                })()}
          ...
          <div onClick={() => setShowLiveModal(false)} className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            ...
            >
              ...
              <form onSubmit={(e) => { e.preventDefault(); saveLiveClassMutation.mutate(); }} className="space-y-4">
                ...
                <button type="submit" disabled={saveLiveClassMutation.isPending} className="w-full py-2.5 bg-primary text-primary-foreground font-bold rounded-xl flex items-center justify-center gap-1.5">
                  <Save size={12} />
