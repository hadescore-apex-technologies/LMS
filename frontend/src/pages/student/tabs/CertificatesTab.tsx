...
import { Award, Download, ShieldCheck, Sparkles, CheckCircle2, Lock } from 'lucide-react';
       ...
       >
        {certificates.length === 0 ? (
          ...
          certificates.map(cert => {
            const isUnlocked = cert.is_unlocked !== false;
            ...
            return (
              <motion.div 
                ...
                className={`relative rounded-2xl cyber-glass-card p-4 shadow-sm transition-all duration-200 flex flex-col justify-between group overflow-hidden ${
                  isUnlocked 
                    ? 'border-amber-500/40 hover:border-amber-400 hover:shadow-[0_0_25px_rgba(245,158,11,0.25)]' 
                    : 'border-slate-800/80 opacity-80'
                }`}
              >
              {/* Gold Ambient Shimmer Glow (only when unlocked) */}
              {isUnlocked && (
              ...
              <div className="space-y-2.5 relative z-10">
                <div className="flex justify-between items-center">
                  {isUnlocked ? (
                    ...
                    <div className="h-9 w-9 bg-slate-900 border border-slate-800 text-slate-500 rounded-xl flex items-center justify-center shadow-sm shadow-black/40">
                      <Lock size={16} />
                    </div>
                  )}

                  {isUnlocked ? (
                    <span className="flex items-center gap-1 text-[9px] font-black uppercase tracking-wider text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2 py-0.5 rounded-full animate-pulse">
                      <ShieldCheck size={11} /> Verified
                    </span>
                  ) : (
                    <span className="flex items-center gap-1 text-[9px] font-black uppercase tracking-wider text-slate-400 bg-slate-500/10 border border-slate-700/50 px-2 py-0.5 rounded-full">
                      <Lock size={10} /> Locked
                    </span>
                  )}
                </div>
                ...
                <div>
                  <span className={`text-[8px] uppercase font-black tracking-wider block ${isUnlocked ? 'text-amber-400' : 'text-slate-500'}`}>Official Credential</span>
                  <h3 className={`font-extrabold text-xs leading-snug truncate ${isUnlocked ? 'text-foreground group-hover:text-amber-400 transition-colors' : 'text-muted-foreground'}`}>{cert.course_title}</h3>
                </div>
                ...
                <div className="bg-muted/40 p-2 rounded-xl border border-border/80">
                  ...
                  <span className={`font-mono font-black tracking-wider select-all block text-[11px] truncate ${isUnlocked ? 'text-cyan-400' : 'text-slate-600'}`}>{cert.certificate_code}</span>
                </div>
              </div>
              ...
              <div className="pt-2.5 border-t border-border/60 flex flex-col gap-2 relative z-10 mt-2">
                ...
                {isUnlocked ? (
                  ...
                  <button
                    type="button"
                    disabled
                    className="w-full py-1.5 bg-slate-950 border border-slate-900 text-slate-600 font-extrabold rounded-xl text-center flex items-center justify-center gap-1.5 text-[11px] cursor-not-allowed opacity-50"
                  >
                    <Lock size={12} />
                    <span>Locked (Requires 100% Course Progress)</span>
                  </button>
                )}
              </div>
            </motion.div>
