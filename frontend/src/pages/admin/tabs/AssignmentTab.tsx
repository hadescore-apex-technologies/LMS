...
export const AssignmentTab: React.FC = () => {
  ...
  const { data: submissions = [] } = useQuery<Submission[]>({
    queryKey: ['staff-submissions-list', liveMode],
    queryFn: async () => {
  ...
  const { data: assignments = [], refetch: refetchAssignments } = useQuery<Assignment[]>({
    queryKey: ['admin-assignments-list', liveMode],
                  ...
                  <tr key={assign.id} className="hover:bg-muted/10 transition-colors">
                    ...
                    <td className="py-3.5 px-4 text-right">
                      <div className="inline-flex items-center gap-1.5">
                        <button
                          onClick={() => {
                            const taskSubs = submissions.filter(s => s.assignment_title === assign.title || (s as any).assignment === assign.id);
                            if (taskSubs.length === 0) {
                              toast.error(`No student submissions received for "${assign.title}" yet.`);
                            } else if (taskSubs.length === 1) {
                              openGradeModal(taskSubs[0]);
                            } else {
                              setViewTaskSubmissionsAssignment(assign);
                            }
                          }}
                          ...
                          title="Open Grading Form directly"
                        >
                          <FileText size={12} />
                          <span>Grade Submissions ({submissions.filter(s => s.assignment_title === assign.title || (s as any).assignment === assign.id).length})</span>
                        </button>
                        <button
