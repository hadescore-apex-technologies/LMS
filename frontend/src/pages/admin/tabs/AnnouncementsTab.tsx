...
import { Megaphone, Plus, Trash2, X, Send, Clock, Users } from 'lucide-react';
...
export const AnnouncementsTab: React.FC = () => {
  ...
  const [targetRole, setTargetRole] = useState<'ALL' | 'STAFF' | 'STUDENT'>('ALL');
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setIsSending(true);
    try {
      // 1. Send real broadcast via DB notifications API
      const res = await api.post('analytics/broadcast/', {
        title: title.trim(),
      ...
      const newAnn: Announcement = {
        ...
        content,
        priority,
        scope: targetRole === 'ALL' ? 'All Platform Users' : targetRole === 'STAFF' ? 'Mentors & Staff' : 'Students Only',
      ...
      toast.success(res.data.message || 'Announcement broadcasted to all users!');
    } catch {
      toast.error('Failed to broadcast announcement to users.');
    } finally {
      setIsSending(false);
    }
  };
  ...
  const handleDelete = (id: number) => {
    ...
    saveAnnouncements(updated);
    toast.success('Announcement removed from log.');
  };
    ...
    <div className="space-y-6 text-xs">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Broadcast high-priority notices, schedule guidelines, and publish core alerts directly to user notifications.</p>
        </div>
        <button
        ...
        {announcements.map(ann => (
          <div key={ann.id} className="p-5 bg-card border border-border rounded-2xl shadow-sm space-y-3 relative hover:border-primary/20 transition-all">
            ...
            <div className="flex flex-wrap gap-2 items-center text-[10px] text-muted-foreground">
              ...
              <span className="px-2 py-0.5 rounded bg-primary/10 border border-primary/20 text-primary font-bold uppercase tracking-wider flex items-center gap-1">
                <Users size={10} />
                {ann.scope}
              </span>
            ...
            >
              ...
              <form onSubmit={handleCreate} className="space-y-4">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  <div>
                    <label className="block text-[10px] text-muted-foreground uppercase mb-1 font-bold">Priority</label>
                  ...
                  <div>
                    <label className="block text-[10px] text-muted-foreground uppercase mb-1 font-bold">Target Audience</label>
                    <select value={targetRole} onChange={(e) => setTargetRole(e.target.value as any)} className="w-full h-10 px-3 bg-muted/40 border border-border rounded-xl outline-none font-semibold">
                      <option value="ALL">All Users (Staff + Students)</option>
                      <option value="STUDENT">Students Only</option>
                      <option value="STAFF">Mentors / Staff Only</option>
                    </select>
                  </div>
                </div>
                <div>
                  <label className="block text-[10px] text-muted-foreground uppercase mb-1 font-bold">Message Content *</label>
                ...
                <button type="submit" disabled={isSending} className="w-full py-2.5 bg-primary text-primary-foreground font-bold rounded-xl flex items-center justify-center gap-1.5 shadow-md shadow-primary/10 disabled:opacity-50">
                  <Send size={12} />
                  <span>{isSending ? 'Broadcasting...' : 'Broadcast Alert'}</span>
                </button>
              </form>
            </motion.div>
          </div>
