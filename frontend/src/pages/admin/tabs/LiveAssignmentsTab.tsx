...
export const LiveAssignmentsTab: React.FC = () => {
  ...
  // 1. Fetch Submissions (Live Mode Only)
  const { data: submissions = [] } = useQuery<Submission[]>({
    queryKey: ['admin-live-submissions-list'],
    queryFn: async () => {
  ...
  const gradeMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-live-submissions-list'] });
  ...
  return (
    <div className="space-y-6 text-xs animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight">Live Assignments Hub</h1>
          <p className="text-muted-foreground text-sm mt-1">Review live student submissions, grade homework deliverables, and monitor tasks assigned by Mentors.</p>
          {activeSubTab === 'submissions' && (
            <span className="inline-flex items-center gap-1.5 mt-2 px-3 py-1 rounded-full bg-emerald-50 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-300 border border-emerald-300 dark:border-emerald-700/60 text-[10px] font-extrabold uppercase tracking-wider shadow-xs">
              ...
              Live Submissions Inbox — All Students
            </span>
          )}
        </div>
        ...
        <span className="font-bold text-muted-foreground block">
          {activeSubTab === 'submissions' ? (
            selectedStudentData 
              ...
              : `Total Students Submitted: ${studentsList.length} profiles`
          ) : (
            `Total Live Assignments: ${assignments.length} tasks`
          )}
        </span>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            {filteredStudents.length === 0 && (
              <div className="sm:col-span-2 lg:col-span-3 py-16 text-center text-muted-foreground bg-card border border-dashed border-border rounded-2xl">
                No live student submissions found.
              </div>
            )}
          </div>
        )
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
              ...
              <div className="relative z-10 space-y-3">
                <div className="flex justify-between items-start gap-2">
                  <div className="flex gap-1 shrink-0 ml-auto">
                    <button onClick={() => { if (window.confirm('Delete assignment?')) deleteAssignmentMutation.mutate(a.id); }} className="p-1.5 bg-destructive/10 rounded-lg text-destructive hover:bg-destructive hover:text-destructive-foreground transition-colors" title="Delete"><Trash2 size={12}/></button>
                  </div>
                </div>
                  ...
                  <div className="flex items-start gap-2 text-[11px] font-medium text-muted-foreground">
                    ...
                    <span className="line-clamp-2">
                      {a.students_details && a.students_details.length > 0 
                        ? `Target: ${a.students_details.map(s => s.name).join(', ')}`
                        : (a.students && a.students.length > 0 ? `${a.students.length} Target Students` : 'Open to All Enrolled')}
                    </span>
                  </div>
                </div>
              </div>
          ...
          {filteredAssignments.length === 0 && !isAssignmentsLoading && (
            <div className="col-span-full py-20 text-center text-muted-foreground bg-card border border-dashed border-border rounded-2xl">
              ...
              <p>No assignments have been created by staff mentors yet.</p>
            </div>
          )}
        </div>
      )}
